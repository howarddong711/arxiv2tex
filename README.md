...
`paper2tex` is an arXiv-first paper cache for coding agents such as Codex, Claude Code, and OpenCode.
...
```powershell
paper2tex resolve "请你阅读这篇论文《attention is all you need》"
```

Inspect how a free-form prompt is interpreted:

```powershell
paper2tex interpret-prompt "帮我参考 attention is all you need 的实验部分写法"
```

Run the default end-to-end workflow for an agent-facing prompt:

```powershell
paper2tex handle-prompt "帮我参考 attention is all you need 的实验部分写法"
```
...
```powershell
paper2tex resolve "帮我看 attention all you need"
paper2tex handle-prompt "就这篇"
```

To isolate multiple parallel conversations, pass a `session_id`:

```powershell
paper2tex --session-id thread-a resolve "帮我看 attention all you need"
paper2tex --session-id thread-a handle-prompt "就这篇"
paper2tex --session-id thread-b pending-status
```

Prepare and cache the paper:

```powershell
paper2tex prepare "请你阅读这篇论文《attention is all you need》"
```

Read a section using the default `reader` view:

```powershell
paper2tex read-section 1706.03762v7 实验
```

Confirm one paper from an ambiguous candidate list:

```powershell
paper2tex select-candidate "帮我看 attention all you need" 第一个 --no-prepare
```

Inspect the latest pending confirmation state:

```powershell
paper2tex pending-status
```
...
```powershell
paper2tex read-fulltex 1706.03762v7 --view clean --limit 400
```

Extract writing-oriented examples for a section such as abstract, related work, method, or experiments:

```powershell
paper2tex extract-writing 1706.03762v7 实验 --top-k 2
```
...
The default cache location is `.paper-cache` in the current working directory.
